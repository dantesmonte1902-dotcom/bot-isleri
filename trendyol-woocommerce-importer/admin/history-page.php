...
// Filtreleri al
$log_type = isset( $_GET['type'] ) ? sanitize_text_field( $_GET['type'] ) : null;
...
<div class="trendyol-container">
	...
	<div class="trendyol-card">
		<h2><?php echo esc_html__( '📜 Import History', 'trendyol-woocommerce-importer' ); ?></h2>
		...
		<div style="margin-bottom: 25px; padding-bottom: 20px; border-bottom: 1px solid var(--border-color);">
			<form method="get" class="trendyol-form" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; align-items: end;">
				<input type="hidden" name="page" value="trendyol-importer-history">

				<div style="margin-bottom: 0;">
					<label for="type_filter"><?php echo esc_html__( 'Type', 'trendyol-woocommerce-importer' ); ?></label>
					<select id="type_filter" name="type" class="form-control">
						<option value=""><?php echo esc_html__( 'All Types', 'trendyol-woocommerce-importer' ); ?></option>
						<option value="import" <?php selected( $log_type, 'import' ); ?>>
							<?php echo esc_html__( 'Import', 'trendyol-woocommerce-importer' ); ?>
						</option>
						<option value="sync" <?php selected( $log_type, 'sync' ); ?>>
							<?php echo esc_html__( 'Sync', 'trendyol-woocommerce-importer' ); ?>
						</option>
					</select>
				</div>

				<div style="margin-bottom: 0;">
					<label for="status_filter"><?php echo esc_html__( 'Status', 'trendyol-woocommerce-importer' ); ?></label>
					<select id="status_filter" name="status" class="form-control">
						<option value=""><?php echo esc_html__( 'All Status', 'trendyol-woocommerce-importer' ); ?></option>
						<option value="success" <?php selected( $status, 'success' ); ?>>
							<?php echo esc_html__( 'Success', 'trendyol-woocommerce-importer' ); ?>
						</option>
						<option value="error" <?php selected( $status, 'error' ); ?>>
							<?php echo esc_html__( 'Error', 'trendyol-woocommerce-importer' ); ?>
						</option>
						<option value="pending" <?php selected( $status, 'pending' ); ?>>
							<?php echo esc_html__( 'Pending', 'trendyol-woocommerce-importer' ); ?>
						</option>
					</select>
				</div>

				<button type="submit" class="btn btn-primary" style="margin-bottom: 0;">
					<?php echo esc_html__( '🔍 Filter', 'trendyol-woocommerce-importer' ); ?>
				</button>
			</form>
		</div>
				...
				<table class="wp-list-table widefat fixed striped">
					<thead>
						<tr>
							...
							<th width="15%"><?php echo esc_html__( 'Type', 'trendyol-woocommerce-importer' ); ?></th>
							<th width="15%"><?php echo esc_html__( 'Action', 'trendyol-woocommerce-importer' ); ?></th>
							<th width="12%"><?php echo esc_html__( 'Status', 'trendyol-woocommerce-importer' ); ?></th>
							<th width="20%"><?php echo esc_html__( 'Product', 'trendyol-woocommerce-importer' ); ?></th>
							<th width="15%"><?php echo esc_html__( 'Date', 'trendyol-woocommerce-importer' ); ?></th>
							<th width="13%"><?php echo esc_html__( 'Actions', 'trendyol-woocommerce-importer' ); ?></th>
						</tr>
					</thead>
					<tbody>
						<?php foreach ( $logs as $log ) : ?>
							<?php
							$product = get_post( $log->product_id );
							...
							<tr>
								...
								<td>
									...
									<?php else : ?>
										<?php if ( ! empty( $log->product_url ) ) : ?>
											<a href="<?php echo esc_url( $log->product_url ); ?>" target="_blank" title="View on Trendyol">
												🔗 Trendyol
											</a>
										<?php else : ?>
											—
								...
								<td>
									<button type="button" class="button button-small" onclick="showLogDetails(<?php echo esc_attr( json_encode( $log ) ); ?>)">
										<?php echo esc_html__( 'Details', 'trendyol-woocommerce-importer' ); ?>
									</button>
								</td>
							</tr>
						<?php endforeach; ?>
		...
		<?php else : ?>
			<div class="alert alert-warning">
				...
				<div>
					<strong><?php echo esc_html__( 'No logs found', 'trendyol-woocommerce-importer' ); ?></strong>
					<p><?php echo esc_html__( 'No import or sync operations have been performed yet.', 'trendyol-woocommerce-importer' ); ?></p>
				</div>
			</div>
		<?php endif; ?>
	</div>
...
<div id="logDetailsModal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.7); z-index:9999; align-items:center; justify-content:center;">
	<div class="trendyol-card" style="max-width:600px; margin:20px;">
		...
		<h3><?php echo esc_html__( 'Log Details', 'trendyol-woocommerce-importer' ); ?></h3>
...
function showLogDetails(log) {
	...
	content.innerHTML = `
		<p><strong><?php echo esc_html__( 'Type:', 'trendyol-woocommerce-importer' ); ?></strong> ${log.log_type}</p>
		<p><strong><?php echo esc_html__( 'Action:', 'trendyol-woocommerce-importer' ); ?></strong> ${log.action}</p>
		<p><strong><?php echo esc_html__( 'Status:', 'trendyol-woocommerce-importer' ); ?></strong> ${log.status}</p>
		<p><strong><?php echo esc_html__( 'Message:', 'trendyol-woocommerce-importer' ); ?></strong> ${log.message}</p>
		${log.product_url ? `<p><strong><?php echo esc_html__( 'Product URL:', 'trendyol-woocommerce-importer' ); ?></strong> <a href="${log.product_url}" target="_blank">${log.product_url}</a></p>` : ''}
		${oldData ? `<p><strong><?php echo esc_html__( 'Old Data:', 'trendyol-woocommerce-importer' ); ?></strong> ${oldData}</p>` : ''}
		${newData ? `<p><strong><?php echo esc_html__( 'New Data:', 'trendyol-woocommerce-importer' ); ?></strong> ${newData}</p>` : ''}
	`;
