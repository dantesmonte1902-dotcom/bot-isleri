...
if ( isset( $_POST['bamkur_save'], $_POST['bamkur'] ) ) {
check_admin_referer( 'trendyol_bam_manual_save' );
$kargo_service->save_manual_bam_rate( sanitize_text_field( wp_unslash( $_POST['bamkur'] ) ) );
$_SESSION['kargo_notice'] = 'BAM kuru manuel olarak güncellendi!';
wp_redirect( admin_url( 'admin.php?page=trendyol-importer&tab=kargo' ) );
exit;
}

if ( isset( $_POST['currency_save'], $_POST['price_currency'] ) ) {
check_admin_referer( 'trendyol_currency_save' );
$kargo_service->save_currency_setting( sanitize_key( wp_unslash( $_POST['price_currency'] ) ) );
$_SESSION['kargo_notice'] = 'Para birimi güncellendi!';
wp_redirect( admin_url( 'admin.php?page=trendyol-importer&tab=kargo' ) );
exit;
}
...
$active_currency = trendyol_get_active_currency();
...
<h2 style="margin-top:0;">🌍 Fiyat Para Birimi Seçimi</h2>
<form method="post">
<?php wp_nonce_field( 'trendyol_currency_save' ); ?>
<p style="color:#64748b; font-size:13px; margin-top:0;">Ürünler siteye eklenirken fiyatlar seçtiğiniz para birimine çevrilecektir.</p>
<div style="display:flex; flex-direction:column; gap:10px; margin-bottom:16px;">
<label style="display:flex; align-items:center; gap:10px; cursor:pointer; padding:10px 14px; border-radius:8px; border:2px solid <?php echo 'rsd' === $active_currency ? '#2563eb' : '#e2e8f0'; ?>; background:<?php echo 'rsd' === $active_currency ? '#eff6ff' : '#fff'; ?>;">
<input type="radio" name="price_currency" value="rsd" <?php checked( $active_currency, 'rsd' ); ?> style="accent-color:#2563eb;">
<span>
<b>🇷🇸 RSD — Sırp Dinarı</b><br>
<small style="color:#64748b;">Geçerli kur: <b><?php echo esc_html( function_exists( 'get_trendyol_rsd_kuru' ) ? get_trendyol_rsd_kuru() : '—' ); ?></b> RSD/EUR</small>
</span>
</label>
<label style="display:flex; align-items:center; gap:10px; cursor:pointer; padding:10px 14px; border-radius:8px; border:2px solid <?php echo 'bam' === $active_currency ? '#2563eb' : '#e2e8f0'; ?>; background:<?php echo 'bam' === $active_currency ? '#eff6ff' : '#fff'; ?>;">
<input type="radio" name="price_currency" value="bam" <?php checked( $active_currency, 'bam' ); ?> style="accent-color:#2563eb;">
<span>
<b>🇧🇦 KM/BAM — Bosna Markı</b><br>
<small style="color:#64748b;">Geçerli kur: <b><?php echo esc_html( $bam_kur ); ?></b> KM/EUR</small>
</span>
</label>
<label style="display:flex; align-items:center; gap:10px; cursor:pointer; padding:10px 14px; border-radius:8px; border:2px solid <?php echo 'eur' === $active_currency ? '#2563eb' : '#e2e8f0'; ?>; background:<?php echo 'eur' === $active_currency ? '#eff6ff' : '#fff'; ?>;">
<input type="radio" name="price_currency" value="eur" <?php checked( $active_currency, 'eur' ); ?> style="accent-color:#2563eb;">
<span>
<b>💶 EUR — Euro</b><br>
<small style="color:#64748b;">Kargo ve marj uygulanır, sabit 1 EUR = 1 EUR</small>
</span>
</label>
</div>
<button type="submit" name="currency_save" class="button button-primary" style="width:100%;">Para Birimini Kaydet</button>
</form>
</div>
...
<h2 style="margin-top:0;">🇧🇦 BAM Kuru (KM) Ayarı</h2>
<form method="post" style="display:flex; gap:6px; align-items:center;">
<?php wp_nonce_field( 'trendyol_bam_manual_save' ); ?>
<label for="bamkur" style="font-weight:600;">Manuel BAM Kuru:</label>
<input type="number" name="bamkur" id="bamkur" step="0.0001" min="0" value="<?php echo esc_attr( $bam_kur ); ?>" required style="width:120px; margin-left:7px;">
<button type="submit" name="bamkur_save" class="button button-primary" style="margin-left:5px;">Kaydet</button>
</form>
<div style="margin-top:12px;color:#64748b;">
<b>Geçerli Kur:</b> <?php echo esc_html( $bam_kur ); ?><br>
<span style="font-size:12px;">1 EUR = <?php echo esc_html( $bam_kur ); ?> KM. Sabit kur olduğu için TCMB'den çekilmez.</span>
</div>
</div>
